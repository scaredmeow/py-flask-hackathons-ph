...
def paginated_response(
    ...
    def inner(f):
        ...
        def paginate(*args, **kwargs):
            ...
            if filter_args:
                select_query = select_query.filter(*[sqla.text(arg) for arg in filter_args]).params(
                    {"query_args" + str(index + 1): arg for index, arg in enumerate(filter_values)}
                )
        ...
        return arguments(pagination_schema)(
            response(PaginatedCollection(schema, pagination_schema=pagination_schema))(paginate)
        )
